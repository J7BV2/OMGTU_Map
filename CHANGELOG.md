...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest "omgtu_map" -- --template=react-ts  --no-rolldown --no-immediate --eslint`.
- Обновление `vite.config.ts` с портом.
- Создание файла проекта (`omgtu_map.esproj`).
